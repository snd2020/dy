...
ImageWrapper.prototype.findColor = function (image1, color, threshold, x, y, ex, ey, limit, orz) {
    ...
    color = this.convertFirstColorArrayToString(color);
 ...
 *
 * @param image1 图片
 * @param points     字符串类似这样 6|1|0x969696-0x000010
 * @param threshold  找色时颜色相似度取值为 0.0 ~ 1.0
...
ImageWrapper.prototype.cmpColor = function (image1, points, threshold, x, y, ex, ey) {
    ...
    points = this.convertMultiColorArrayToString(points);
    let ad = [points];
    let index = imageWrapper.cmpMultiColor(image1.uuid, JSON.stringify(ad), threshold, x, y, ex - x, ey - y);
    if (index === -1) {
        return false;
    }
    return true;
...
ImageWrapper.prototype.cmpColorEx = function (points, threshold, x, y, ex, ey) {
    ...
    let ad = [points];
    let index = imageWrapper.cmpMultiColorCurrentScreen(JSON.stringify(ad), threshold, x, y, ex - x, ey - y);
    if (index === -1) {
        return false;
    }
    return true;
